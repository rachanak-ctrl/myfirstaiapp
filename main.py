...
# 1. FIXED PROMPT: We add strict instructions so it doesn't give you JSON or extra chatter
tweet_template = """
...
topic = st.text_input("Topic", placeholder="e.g., Remote work, Coffee, AI coding")
...
if st.button("Generate"):
    if not topic.strip():
        ...
        with st.spinner("Writing your tweets..."):
            ...
            # 2. FIXED DISPLAY: Split the response by our custom marker and clean it up
            ...
            for i, tweet in enumerate(cleaned_tweets, 1):
                # Using st.info or st.chat_message makes it look like a real tweet box
                st.info(f"**Tweet #{i}**\n\n{tweet}")
